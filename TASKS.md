...
- [x] [done 2026-04-23] **Drop `./dist` positional arg from deploy scripts** — `yarn prod` was `deno deploy --org ... --app janzheng --prod ./dist`. The positional is CLI's upload root, NOT an app-directory hint — so `./dist` uploaded only build artifacts (no `deno.json`) and the Astro preset's `deno task build` failed. Fixed in tandem with labspace after the cross-deploy P0. See `CLAUDE.md` for the zapping-zero canonical reference. #deploy
- [x] [done 2026-04-23] **Resume system documentation** — documented the `/resume?id=...&state=split` JSON-driven resume workflow. `RESUME.md` at root is the practical "how to update my CV" cheat sheet; `.brief/resume-system.md` is the deep technical reference (schema, sheet integration, extending guide). #docs
- [x] [done 2026-04-23] **Cross-fork cruft sweep** — deleted 7 leftover files from labspace fork inheritance: `src/pages/test.astro`, `src/components/projects/{TestProject.astro, test.md}`, `src/data/{test.ts, resume.json, resume-schema-3.json}`, `src/components/deno/GeminiBbox.astro`. Orphan commented-out `<GeminiBbox />` in `JanZheng.astro` also cleaned. `cv.json` is the canonical resume; all other resume JSONs were unreferenced. #cleanup #fork-cruft
