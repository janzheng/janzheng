...
<div class="Converter basic-converter | py-4 px-2 rounded-md bg-sky-50 mb-4 shadow-sm shadow-blue-200/80 | border border-blue-100">
    ...
    <Popover.Root bind:open={calOpen}>
      <Popover.Trigger asChild let:builder>
        <Button
          variant="outline"
          class={cn(
            "hidden sm:block | flex-1 px-1 md:px-4 md:w-[240px] justify-start text-left font-normal overflow-hidden | border-blue-200 hover:border-blue-300 hover:bg-white/40 focus-visible:ring-blue-400/100",
            !$date$ && "text-muted-foreground"
          )}
          builders={[builder]}
        >
          <CalendarIcon class="inline mr-2 h-4 w-4" />
