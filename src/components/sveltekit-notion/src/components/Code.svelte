{#if false}
    <slot />
{/if}
<div id={`_block-${block.id}`} class="line-numbers notion-code bg-[#002B36]">
  <code
    bind:this={highlight}
    class={`language-${language?.toLowerCase() || 'javascript'}`}>{code}
  </code>
</div>
...
<script>
    ...
    const language = block.properties
        ? block.properties?.language?.[0]?.[0] || block.properties?.language?.[0]
        : 'javascript'
